...
def prex(flight_id):
    ...
    if ourflight is None :
        return jsonify({"error": "can't find the flight"}),422
    pass_flights = ourflight.passenger
    listPasses = []
    for passeng in pass_flights:
        listPasses.append(passeng.name)
    jsod = {
       "id" : ourflight.id,
       "origin" : ourflight.origin,
       "destination" : ourflight.destination,
       "passengers" : listPasses
    }
    return jsonify(jsod),200
